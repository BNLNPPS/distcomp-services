...
| Templates | | one line in `panda_server-httpd.conf` (below); the systemd units, the logrotate files and the sysconfig templates are unchanged, so no `daemon-reload` is needed. The official installation guides (panda-wms.readthedocs.io, server and JEDI pages, admin guide, database administration) carry no upgrade procedure beyond these elements: the pip install from git, the `.rpmnew` templates beside the live files, the units and logrotate files linked from the tree, and the schema check at start; the JEDI page's `pip install panda-jedi` predates JEDI's move into panda-server (2025-08-28; panda-jedi archived 2025-09-02) and does not apply. |
